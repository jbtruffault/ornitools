...
//Source: https://github.com/diragb/shadcn-dropzone/tree/master
...
const Dropzone = ({
  ...
  const dropzone = useDropzone({
    ...
    onDrop(acceptedFiles, fileRejections, event) {
      ...
      setFilesUploaded([...filesUploaded, acceptedFiles[0]]);
  ...
  const deleteUploadedFile = (index: number) => {
    setFilesUploaded([]); // Effacer le fichier téléchargé
  };
        ...
        {children ? (
          ...
          <div className='flex items-center flex-col gap-1.5'>
            <div className='flex items-center flex-row gap-0.5 text-sm font-medium'>
              <Upload className='mr-2 h-4 w-4' /> Sélectionner le fichier
            </div>
            {props.maxSize && (
              ...
              >
                <div className='flex items-center flex-row gap-4 h-full'>
                  {fileUploaded.type === 'application/pdf' ? (
                    <PDF 
                      className='text-rose-700 w-6 h-6'
                      alt="PDF Icon" 
                      />
                  ) : (
                    <Image 
